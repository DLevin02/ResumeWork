...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        if (!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("Start and/or End Not Found");
        }

        PriorityQueue<Path> order = new PriorityQueue<Path>();
        Hashtable<Vertex, Vertex> beenTo = new Hashtable<Vertex, Vertex>();
        order.add(new Path(vertices.get(start)));

        while (!order.isEmpty()) {
            Path pathOfCurr = order.remove();
            Vertex vertexCurrent = pathOfCurr.end;
            while (beenTo.containsKey(vertexCurrent)) {
                if (!order.isEmpty()) {
                    pathOfCurr = order.remove();
                    vertexCurrent = pathOfCurr.end;
                } else {
                    throw new NoSuchElementException("Path From Start To End Not Found");
                }
            }

            if (pathOfCurr.end.data.equals(end)) {
                return pathOfCurr;
            }
            beenTo.put(vertexCurrent, vertexCurrent);

            for (Edge curEdge : vertexCurrent.edgesLeaving) {
                Path newPath = new Path(pathOfCurr, curEdge);
                order.add(newPath);
            }
        }
        throw new NoSuchElementException("Path From Start To End Not Found");
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            List<T> pastSeq = copyPath.dataSequence;
            this.dataSequence = new LinkedList<>();
            for (T curData : pastSeq) {
