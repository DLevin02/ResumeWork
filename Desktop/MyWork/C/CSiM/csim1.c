...
void init_cache() {
    ...
    for(int i=0; i<S; i++) { // each line allocation
    ...
    for(int m=0; m<S; m++) { // for each set
	for(int n=0; n<E; n++) { // for each line
	    // initialize
	    cache[m][n].valid = 0;
	    cache[m][n].tag = 0;
	    cache[m][n].track = 0;
...
void free_cache() {
    ...
    free(cache); //free the cache itself too
}  
...
void access_data(mem_addr_t addr) {
	...
	// check if one line is not used yet
    ...
    for(int k=0; k<E; k++) { // for each line
